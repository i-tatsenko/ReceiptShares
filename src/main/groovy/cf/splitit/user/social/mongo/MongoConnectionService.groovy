...
@CompileStatic
class MongoConnectionService implements ConnectionService {

    private final MongoTemplate mongoTemplate
    private final ConnectionConverter converter

    @Autowired
    MongoConnectionService(MongoTemplate mongoTemplate, ConnectionConverter converter) {
    ...
    List<Connection<?>> getConnections(String userId) {
        ...
        Query q = query(where("userId").is(userId))
        q.with(new Sort(ASC, "providerId", "rank"))
