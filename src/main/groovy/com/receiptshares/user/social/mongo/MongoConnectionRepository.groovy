...
class MongoConnectionRepository implements ConnectionRepository {
     ...
     */
    @Override
    MultiValueMap<String, Connection<?>> findConnectionsToUsers(MultiValueMap<String, String> providerUsers) {
    ...
    <A> Connection<A> getPrimaryConnection(Class<A> apiType) {
        String providerId = getProviderId(apiType)
        Connection<A> connection = (Connection<A>) findPrimaryConnectionForProvider(providerId)
        if (connection == null) {
    ...
    <A> Connection<A> findPrimaryConnection(Class<A> apiType) {
        String providerId = getProviderId(apiType)
        return (Connection<A>) findPrimaryConnectionForProvider(providerId)
    }
    ...
    private Connection<?> findPrimaryConnectionForProvider(String providerId) {
